...
public class Table {
	public static void main(String[] arges) {
		...
		// EPL: Table
		String CreateTable = "create table SensorTable(" + " Sensorid String primary key ," + " name String, "
				+ " theCount count(*) )";

		String insertIntoTable = "into table SensorTable select id , count(*) as theCount from Sensor group by id ";
		String OnSelect = "on tempSensor(value <110)  select Sensorid, theCount from SensorTable ";

		admin.createEPL(CreateTable);
		admin.createEPL(insertIntoTable);
		EPStatement state4 = admin.createEPL(OnSelect);
		state4.addListener(new AggergationListener());
