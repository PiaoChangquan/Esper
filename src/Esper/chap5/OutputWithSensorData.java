...
public class OutputWithSensorData {
	public static void main(String[] arges){
		...
		// Epl output
		// Syntax: output [after suppression_def] [[all | first | last |
		// snapshot] every time_period | output_rate events]

		// String OutputEPL = "select avg(value),* from tempSensor.win:time(12
		// sec) "
		...
		// Epl output with all
		String OutputEPL = "select * from tempSensor.win:length(15) " + "output  snapshot every 2 events";
		EPStatement stateOutputEPL = admin.createEPL(OutputEPL);
		stateOutputEPL.addListener(new GeneralListener());
